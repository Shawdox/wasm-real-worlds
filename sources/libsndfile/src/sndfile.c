...
{	int		error, format ;
	...
	printf("\e[32m[In psf_open_file() switch]\e[0m psf->sf.format=%d, SF_CODEC (psf->sf.format)=%d\n", psf->sf.format, SF_CODEC(psf->sf.format));
	switch (SF_CODEC (psf->sf.format))
	...
	/* Call the initialisation function for the relevant file type. */
		printf("\e[32m[In psf_open_file() switch]\e[0m psf->sf.format=%d, SF_CONTAINER(psf->sf.format)=%d\n", psf->sf.format, SF_CONTAINER(psf->sf.format));
